...
namespace Waher.Content.Markdown.Test
{
	...
	{
		private void DoTest(string MarkdownFileName, string XamarinFormsFileName)
		{
			string Markdown = File.ReadAllText("Markdown/Syntax/" + MarkdownFileName);
			string ExpectedText = File.ReadAllText("XF/" + XamarinFormsFileName);
			...
			string GeneratedXaml = Doc.GenerateXamarinForms(XML.WriterSettings(true, true));
