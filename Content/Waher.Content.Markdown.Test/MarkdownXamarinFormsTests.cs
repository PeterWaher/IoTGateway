...
namespace Waher.Content.Markdown.Test
{
	...
	{
		private async Task DoTest(string MarkdownFileName, string XamarinFormsFileName)
		{
			string Markdown = await Resources.ReadAllTextAsync("Markdown/Syntax/" + MarkdownFileName);
			string ExpectedText = await Resources.ReadAllTextAsync("XF/" + XamarinFormsFileName);
		...
		{
			await this.DoTest("Test_32_TablesAndNotes.md", "Test_32_TablesAndNotes.xml");
		}
	}
}
