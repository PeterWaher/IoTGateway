...
namespace Waher.Content.Markdown.Model.BlockElements
{
	...
	/// </summary>
	public class RightAligned : BlockElementChildren
	{
		/// <summary>
		/// Represents a right-aligned set of blocks in a markdown document.
		/// </summary>
		...
		public RightAligned(MarkdownDocument Document, IEnumerable<MarkdownElement> Children)
		...
		{
			await SuffixedBlock(Output, this.Children, ">>");
			Output.AppendLine();
		}
		...
		{
			Output.AppendLine("<div class='horizontalAlignRight'>");
		...
		public override async Task GenerateXAML(XmlWriter Output, TextAlignment TextAlignment)
		{
			...
			foreach (MarkdownElement E in this.Children)
				await E.GenerateXAML(Output, TextAlignment.Right);
		...
		{
			Output.AppendLine("\\begin{flushright}");

			foreach (MarkdownElement E in this.Children)
				await E.GenerateLaTeX(Output);

			Output.AppendLine("\\end{flushright}");
			Output.AppendLine();
		}
		...
		{
			this.Export(Output, "RightAligned");
		}
		...
		{
			return new RightAligned(Document, Children);
		}
		...
		{
			Statistics.NrRightAlignedBlocks++;
		}
	}
}
