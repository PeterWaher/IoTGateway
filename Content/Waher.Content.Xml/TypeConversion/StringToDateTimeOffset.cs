...
{
	/// <summary>
	/// Converts a <see cref="string"/> to a <see cref="DateTimeOffset"/>.
	/// </summary>
	public class StringToDateTimeOffset : ITypeConverter
	{
		/// <summary>
		/// Converts a <see cref="string"/> to a <see cref="DateTimeOffset"/>.
		/// </summary>
		public StringToDateTimeOffset()
		{
		}
		...
		/// </summary>
		public Type To => typeof(DateTimeOffset);
		...
		public bool TryConvert(object Value, out object Result)
		{
			...
			{
				if (XML.TryParse(s, out DateTimeOffset TP))
				{
					Result = TP;
					return true;
				}
				else if (DateTimeOffset.TryParse(s, out TP))
				{
					Result = TP;
					return true;
				}
				else if (CommonTypes.TryParseRfc822(s, out TP))
				{
					Result = TP;
					return true;
				}
