		...
		public void Convert(string FromContentType, Stream From, string ToContentType, Stream To)
		{
			...
			try
			{
				for (i = 0; i < c; i++)
				{
					...
					if (i == c - 1)
						this.sequence[i].Value.Convert(FromType, Intermediate, ToContentType, To);
					else
					{
						...
						this.sequence[i].Value.Convert(FromType, Intermediate == null ? From : Intermediate, ToContentType, Intermediate2);
