...
namespace Waher.Groups
{
	...
	public abstract class GroupNode : IGroupNode
	{
		...
		public static implicit operator ThingReference(GroupNode Node)
		{
		...
		[Header(2, "Group ID:", 0)]
		[Page(3, "Group", 0)]
		[ToolTip(4, "Group identity in the collection of groups.")]
		[Required]
		...
		/// </summary>
		[IgnoreMember]
		public string SourceId => GroupSource.SourceID;
		...
		public virtual async Task LogMessageAsync(MessageType Type, string EventId, string Body)
		{
			...
			foreach (GroupMessage Message in await Database.Find<GroupMessage>(new FilterAnd(
			...
			if (!Updated)
			{
				GroupMessage Msg = new GroupMessage(this.objectId, DateTime.Now, Type, EventId, Body)
				{
		...
		{
			await GroupSource.NewEvent(new NodeStatusChanged()
			{
		...
		public virtual async Task<bool> RemoveMessageAsync(MessageType Type, string EventId)
		{
			...
			foreach (GroupMessage Message in await Database.FindDelete<GroupMessage>(new FilterAnd(
				new FilterFieldEqualTo("NodeId", this.objectId),
				new FilterFieldEqualTo("Type", Type),
				new FilterFieldEqualTo("EventId", EventId))))
			{
			...
			if (Removed)
			{
				...
				foreach (GroupMessage Message in await Database.Find<GroupMessage>(new FilterFieldEqualTo("NodeId", this.objectId)))
				{
					switch (Type)
					{
						case MessageType.Error:
		...
		/// <summary>
		/// Gets a Group ID, based on <paramref name="GroupId"/> that is not already available in the database.
		/// </summary>
		/// <param name="GroupId">Desired Group ID</param>
		/// <returns>A Group ID that does not exist in the database.</returns>
		public static async Task<string> GetUniqueGroupId(string GroupId)
		{
			using Semaphore Semaphore = await Semaphores.BeginWrite("Groups." + GroupId);
			...
			while (true)
			{
				if (await Database.FindFirstIgnoreRest<GroupNode>(
					new FilterFieldEqualTo("NodeId", s = GroupId + Suffix)) is null)
				{
					return s;
				}
			...
			where T : INode
		{
			...
			while (!(Loop is null))
			{
				if (Loop is T Ancestor)
					return Ancestor;
				else if (Loop is GroupNode GroupNode)
					Loop = await GroupNode.GetParent();
				else
					Loop = Loop.Parent;
			}
		...
		internal async Task<GroupNode> LoadParent()
		{
			...
			this.parent = await Database.LoadObject<GroupNode>(this.parentId);
			GroupSource.RegisterNode(this.parent);
		...
		{
			return Task.FromResult(Caller.HasPrivilege("Source." + GroupSource.SourceID + ".Node.View"));
		}
		...
		{
			return Task.FromResult(Caller.HasPrivilege("Source." + GroupSource.SourceID + ".Node.Edit"));
		}
		...
		{
			return Task.FromResult(Caller.HasPrivilege("Source." + GroupSource.SourceID + ".Node.Add"));
		}
		...
		{
			return Task.FromResult(Caller.HasPrivilege("Source." + GroupSource.SourceID + ".Node.Destroy"));
		}
		...
		{
			Namespace Namespace = await Language.GetNamespaceAsync(typeof(GroupNode).Namespace)
				?? await Language.CreateNamespaceAsync(typeof(GroupNode).Namespace);

			LinkedList<Parameter> Result = new LinkedList<Parameter>();
			Result.AddLast(new StringParameter("NodeId", await Namespace.GetStringAsync(5, "Group ID"), this.nodeId));
			Result.AddLast(new StringParameter("Type", await Namespace.GetStringAsync(6, "Type"), await this.GetTypeNameAsync(Language)));

			if (!(this.parent is null))
				Result.AddLast(new StringParameter("ParentId", await Namespace.GetStringAsync(7, "Parent ID"), this.parent.nodeId));
			...
			if (!(this.children is null))
			{
				...
				Result.AddLast(new Int32Parameter("NrChildren", await Namespace.GetStringAsync(8, "#Children"), i));
			}

			string s = this.state switch
			{
				NodeState.Information => await Namespace.GetStringAsync(9, "Information"),
				NodeState.WarningUnsigned => await Namespace.GetStringAsync(10, "Unsigned Warning"),
				NodeState.WarningSigned => await Namespace.GetStringAsync(11, "Warning"),
				NodeState.ErrorUnsigned => await Namespace.GetStringAsync(12, "Unsigned Error"),
				NodeState.ErrorSigned => await Namespace.GetStringAsync(13, "Error"),
				_ => null,
			};

			if (!string.IsNullOrEmpty(s))
				Result.AddLast(new StringParameter("State", await Namespace.GetStringAsync(14, "State"), s));

			Result.AddLast(new DateTimeParameter("Created", await Namespace.GetStringAsync(15, "Created"), this.created));

			if (this.updated != DateTime.MinValue)
				Result.AddLast(new DateTimeParameter("Updated", await Namespace.GetStringAsync(16, "Updated"), this.updated));
		...
		{
			IEnumerable<GroupMessage> Messages = await Database.Find<GroupMessage>(
				new FilterFieldEqualTo("NodeId", this.objectId), "Created");
			LinkedList<Message> Result = new LinkedList<Message>();

			foreach (GroupMessage Msg in Messages)
		...
		{
			if (!(await this.GetParent() is GroupNode Parent))
				return false;
			else
		...
		{
			if (!(Child is GroupNode Node))
				throw new Exception("Child must be a group node.");
			...
			if (Node.objectId == Guid.Empty)
			{
				...
				await GroupSource.NewEvent(Event);
			}
			else
		...
		protected virtual async Task NodeUpdated()
		{
			...
			await GroupSource.NewEvent(new NodeUpdated()
			{
			...
			{
				GroupSource.RegisterNewNodeId(this, this.oldId);
		...
		public virtual async Task<bool> RemoveAsync(INode Child)
		{
			...
			if (Node.objectId != Guid.Empty)
			{
				...
				await GroupSource.NewEvent(new NodeRemoved()
				{
					NodeId = Node.NodeId,
					Partition = Node.Partition,
					SourceId = Node.SourceId,
		...
		public async virtual Task DestroyAsync()
		{
			...
			GroupSource.UnregisterNode(this);
		}
