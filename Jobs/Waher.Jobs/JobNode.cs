...
namespace Waher.Jobs
{
	...
	public abstract class JobNode : IJobNode
	{
		...
		public static implicit operator ThingReference(JobNode Node)
		{
		...
		[Header(2, "Job ID:", 0)]
		[Page(3, "Job", 0)]
		[ToolTip(4, "Job identity in the collection of jobs.")]
		[Required]
		...
		/// </summary>
		[IgnoreMember]
		public string SourceId => JobSource.SourceID;
		...
		public virtual async Task LogMessageAsync(MessageType Type, string EventId, string Body)
		{
			...
			foreach (JobMessage Message in await Database.Find<JobMessage>(new FilterAnd(
			...
			if (!Updated)
			{
				JobMessage Msg = new JobMessage(this.objectId, DateTime.Now, Type, EventId, Body)
				{
		...
		{
			await JobSource.NewEvent(new NodeStatusChanged()
			{
		...
		public virtual async Task<bool> RemoveMessageAsync(MessageType Type, string EventId)
		{
			...
			foreach (JobMessage Message in await Database.FindDelete<JobMessage>(new FilterAnd(
				new FilterFieldEqualTo("NodeId", this.objectId),
				new FilterFieldEqualTo("Type", Type),
				new FilterFieldEqualTo("EventId", EventId))))
			{
			...
			if (Removed)
			{
				...
				foreach (JobMessage Message in await Database.Find<JobMessage>(new FilterFieldEqualTo("NodeId", this.objectId)))
				{
					switch (Type)
					{
						case MessageType.Error:
		...
		/// <summary>
		/// Gets a Job ID, based on <paramref name="JobId"/> that is not already available in the database.
		/// </summary>
		/// <param name="JobId">Desired Job ID</param>
		/// <returns>A Job ID that does not exist in the database.</returns>
		public static async Task<string> GetUniqueJobId(string JobId)
		{
			using Semaphore Semaphore = await Semaphores.BeginWrite("Jobs." + JobId);
			...
			while (true)
			{
				if (await Database.FindFirstIgnoreRest<JobNode>(
					new FilterFieldEqualTo("NodeId", s = JobId + Suffix)) is null)
				{
					return s;
				}
			...
			where T : INode
		{
			...
			while (!(Loop is null))
			{
				if (Loop is T Ancestor)
					return Ancestor;
				else if (Loop is JobNode JobNode)
					Loop = await JobNode.GetParent();
				else
					Loop = Loop.Parent;
			}
		...
		internal async Task<JobNode> LoadParent()
		{
			...
			this.parent = await Database.LoadObject<JobNode>(this.parentId);
			JobSource.RegisterNode(this.parent);
		...
		{
			Namespace Namespace = await Language.GetNamespaceAsync(typeof(JobNode).Namespace)
				?? await Language.CreateNamespaceAsync(typeof(JobNode).Namespace);

			LinkedList<Parameter> Result = new LinkedList<Parameter>();
			Result.AddLast(new StringParameter("NodeId", await Namespace.GetStringAsync(13, "Job ID"), this.nodeId));
			Result.AddLast(new StringParameter("Type", await Namespace.GetStringAsync(14, "Type"), await this.GetTypeNameAsync(Language)));

			if (!(this.parent is null))
				Result.AddLast(new StringParameter("ParentId", await Namespace.GetStringAsync(15, "Parent ID"), this.parent.nodeId));
			...
			if (!(this.children is null))
			{
				...
				Result.AddLast(new Int32Parameter("NrChildren", await Namespace.GetStringAsync(16, "#Children"), i));
			}

			string s = this.state switch
			{
				NodeState.Information => await Namespace.GetStringAsync(5, "Information"),
				NodeState.WarningUnsigned => await Namespace.GetStringAsync(6, "Unsigned Warning"),
				NodeState.WarningSigned => await Namespace.GetStringAsync(7, "Warning"),
				NodeState.ErrorUnsigned => await Namespace.GetStringAsync(8, "Unsigned Error"),
				NodeState.ErrorSigned => await Namespace.GetStringAsync(9, "Error"),
				_ => null,
			};

			if (!string.IsNullOrEmpty(s))
				Result.AddLast(new StringParameter("State", await Namespace.GetStringAsync(10, "State"), s));

			Result.AddLast(new DateTimeParameter("Created", await Namespace.GetStringAsync(11, "Created"), this.created));

			if (this.updated != DateTime.MinValue)
				Result.AddLast(new DateTimeParameter("Updated", await Namespace.GetStringAsync(12, "Updated"), this.updated));
		...
		{
			IEnumerable<JobMessage> Messages = await Database.Find<JobMessage>(
		...
		{
			if (!(await this.GetParent() is JobNode Parent))
				return false;
			else
		...
		{
			if (!(Child is JobNode Node))
				throw new Exception("Child must be a job node.");
			...
			if (Node.objectId == Guid.Empty)
			{
				...
				await JobSource.NewEvent(Event);
			}
			else
		...
		protected virtual async Task NodeUpdated()
		{
			...
			await JobSource.NewEvent(new NodeUpdated()
			{
			...
			{
				JobSource.RegisterNewNodeId(this, this.oldId);
		...
		public virtual async Task<bool> RemoveAsync(INode Child)
		{
			...
			if (Node.objectId != Guid.Empty)
			{
				...
				await JobSource.NewEvent(new NodeRemoved()
				{
					NodeId = Node.NodeId,
					Partition = Node.Partition,
					SourceId = Node.SourceId,
