	...
	public class JobSource : IDataSource
	{
		...
		public static async Task<int> DeleteOldEvents(TimeSpan MaxAge)
		{
			...
			if (NrEvents > 0)
			{
				...
				if (NrEvents == 1)
					Log.Informational("Deleting 1 job event from the database.", SourceID, Tags);
				else
					Log.Informational("Deleting " + NrEvents.ToString() + " job events from the database.", SourceID, Tags);
			}
