...
namespace Waher.Networking.HTTP.JsonRpc
{
	...
	public class ProtectedMethod
	{
		...
		public ProtectedMethod(MethodInfo Method, bool CaseSensitive)
			: this(Method, CaseSensitive, JsonRpcWebService.GetRequiredPrivileges(Method))
		{
		}
			...
			bool CaseSensitive, string[]? RequiredPrivileges)
		{
			...
			this.Arguments = new ProtectedMethodArgumentInfo[this.NrArguments];
		...
		/// </summary>
		public HttpAuthenticationScheme[]? AuthenticationMechanisms { get; private set; }
		...
		/// Arguments
		/// </summary>
		public ProtectedMethodArgumentInfo[] Arguments { get; }
			...
			[NotNullWhen(true)] out object?[]? Arguments)
		{
			...
			foreach (KeyValuePair<string, object?> P in Parameters)
			{
				...
				ProtectedMethodArgumentInfo ArgumentInfo = this.Arguments[i];
			...
			{
				foreach (ProtectedMethodArgumentInfo Argument in this.Arguments)
				{
		...
		/// <summary>
		/// Updates the authentication mechanisms available to authenticate users that
		/// want to access the method.
		/// </summary>
		public void UpdateAuthenticationMechanisms()
		{
			this.UpdateAuthenticationMechanisms(null);
		}
		...
		/// want to access the method.
		/// </summary>
		/// <param name="ResourceMetaDataUri">Resource Meta-data URI.</param>
		public void UpdateAuthenticationMechanisms(string? ResourceMetaDataUri)
		{
			if (this.RequiresAuthentication)
			{
				...
				{
					this.AuthenticationMechanisms = HttpModule.GetAuthenticationSchemes(
						new Uri(ResourceMetaDataUri), this.RequiredPrivileges);
				}
			}
		}
	}
