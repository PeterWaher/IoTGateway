...
namespace Waher.Networking.HTTP.ContentEncodings
{
	...
	public class DeflateContentEncoding : IContentEncoding
	{
		...
		/// </summary>
		private class DeflateEncoder : TransferEncoding
		{
			...
			public void PrepareForCompression()
			{
				...
				this.deflateEncoder = new DeflateStream(this.ms, CompressionMode.Compress, true);
			}
			...
			public override async Task<bool> FlushAsync()
			{
				if (this.dataWritten)
				{
					await this.deflateEncoder.FlushAsync();
			...
			public override void Dispose()
			{
				...
				this.deflateEncoder?.Dispose();
				this.deflateEncoder = null;

				this.deflateDecoder?.Dispose();
				this.deflateDecoder = null;
