...
namespace Waher.Networking.XMPP.HTTPX
{
	...
	/// 
	/// For the putter to work, the application needs to register a <see cref="HttpxProxy"/> object instance
	/// as a Model Parameter named HTTPX.
	/// </summary>
	public class HttpxPutter : PutterBase
	{
		/// <summary>
		/// Content Putter, puting content using the HTTPX URI Scheme.
		/// 
		...
		public override async Task<ContentBinaryResponse> PutAsync(Uri Uri, byte[] EncodedData, string ContentType, 
			...
			params KeyValuePair<string, string>[] Headers)
		{
			...
			try
			{
				...
				await HttpxClient.Request(FullJid, "PUT", LocalUrl, 1.1, Headers2, Data, async (Sender, e) =>
					{
						if (e.Ok)
						{
						...
						else
						{
							State.Done.TrySetException((Exception)e.StanzaError ?? 
								new GenericException("Unable to put resource.", null, Uri.OriginalString));
						}
