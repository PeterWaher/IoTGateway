...
namespace Waher.Networking.XMPP.Events
{
	...
	public class PresenceEventArgs : EventArgs
	{
		...
		public async Task Accept()
		{
			if (this.type == PresenceType.Subscribe)
			{
				if (!(this.client is null))
					await this.client.PresenceSubscriptionAccepted(this.id, this.fromBareJid);
				else
					await this.component.PresenceSubscriptionAccepted(this.id, this.to, this.fromBareJid);
			}
			else if (this.type == PresenceType.Unsubscribe)
			{
				if (!(this.client is null))
					await this.client.PresenceUnsubscriptionAccepted(this.id, this.fromBareJid);
				else
					await this.component.PresenceUnsubscriptionAccepted(this.id, this.to, this.fromBareJid);
			}
			else
		...
		public async Task Decline()
		{
			...
			{
				if (!(this.client is null))
					await this.client.PresenceSubscriptionDeclined(this.id, this.fromBareJid);
				else
					await this.component.PresenceSubscriptionDeclined(this.id, this.to, this.fromBareJid);
			}
			else if (this.type == PresenceType.Unsubscribe)
			{
				if (!(this.client is null))
					await this.client.PresenceUnsubscriptionDeclined(this.id, this.fromBareJid);
				else
					await this.component.PresenceUnsubscriptionDeclined(this.id, this.to, this.fromBareJid);
			}
			else
				throw new Exception("Presence stanza is not a subscription or unsubscription.");
		}
