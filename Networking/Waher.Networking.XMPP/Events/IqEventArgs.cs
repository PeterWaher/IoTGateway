...
namespace Waher.Networking.XMPP.Events
{
    ...
    public class IqEventArgs : EventArgs
    {
        ...
        public async Task IqResult(string Xml)
        {
            if (!(this.e2eEncryption is null))
                await this.e2eEncryption.SendIqResult(this.client, E2ETransmission.IgnoreIfNotE2E, this.id, this.from, Xml);
            else if (!(this.client is null))
				await this.client.SendIqResult(this.id, this.from, Xml);
            else
				await this.component.SendIqResult(this.id, this.to, this.from, Xml);
        }
        ...
        /// </summary>
        /// <param name="Xml">XML to embed into the response.</param>
        public async Task IqError(string Xml)
        {
            if (!(this.e2eEncryption is null))
				await this.e2eEncryption.SendIqError(this.client, E2ETransmission.IgnoreIfNotE2E, this.id, this.from, Xml);
            else if (!(this.client is null))
				await this.client.SendIqError(this.id, this.from, Xml);
            else
				await this.component.SendIqError(this.id, this.to, this.from, Xml);
        }
        ...
        public async Task IqError(Exception ex)
        {
            try
            {
                if (!(this.e2eEncryption is null))
					await this.e2eEncryption.SendIqError(this.client, E2ETransmission.IgnoreIfNotE2E, this.id, this.from, ex);
                else if (!(this.client is null))
					await this.client.SendIqError(this.id, this.from, ex);
                else
					await this.component.SendIqError(this.id, this.to, this.from, ex);
            }
            catch (Exception ex2)
            {
                Log.Exception(ex2);
