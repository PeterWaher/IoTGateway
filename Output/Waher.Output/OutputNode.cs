...
namespace Waher.Output
{
	...
	public abstract class OutputNode : IOutputNode
	{
		private Guid objectId = Guid.Empty;
		private Guid parentId = Guid.Empty;
		private OutputNode parent = null;
		...
		public static implicit operator ThingReference(OutputNode Node)
		{
		...
		/// </summary>
		[IgnoreMember]
		public string SourceId => OutputSource.SourceID;
		...
		public virtual async Task LogMessageAsync(MessageType Type, string EventId, string Body)
		{
			...
			foreach (OutputMessage Message in await Database.Find<OutputMessage>(new FilterAnd(
			...
			if (!Updated)
			{
				OutputMessage Msg = new OutputMessage(this.objectId, DateTime.Now, Type, EventId, Body)
				{
		...
		{
			await OutputSource.NewEvent(new NodeStatusChanged()
			{
		...
		public virtual async Task<bool> RemoveMessageAsync(MessageType Type, string EventId)
		{
			...
			foreach (OutputMessage Message in await Database.FindDelete<OutputMessage>(new FilterAnd(
				new FilterFieldEqualTo("NodeId", this.objectId),
				new FilterFieldEqualTo("Type", Type),
				new FilterFieldEqualTo("EventId", EventId))))
			{
			...
			if (Removed)
			{
				...
				foreach (OutputMessage Message in await Database.Find<OutputMessage>(new FilterFieldEqualTo("NodeId", this.objectId)))
				{
					switch (Type)
					{
						case MessageType.Error:
		...
		/// <summary>
		/// Gets a Output ID, based on <paramref name="OutputId"/> that is not already available in the database.
		/// </summary>
		/// <param name="OutputId">Desired Output ID</param>
		/// <returns>A Output ID that does not exist in the database.</returns>
		public static async Task<string> GetUniqueNodeId(string OutputId)
		{
			using Semaphore Semaphore = await Semaphores.BeginWrite("Output." + OutputId);
			...
			while (true)
			{
				if (await Database.FindFirstIgnoreRest<OutputNode>(
					new FilterFieldEqualTo("NodeId", s = OutputId + Suffix)) is null)
				{
					return s;
				}
			...
			where T : INode
		{
			...
			while (!(Loop is null))
			{
				if (Loop is T Ancestor)
					return Ancestor;
				else if (Loop is OutputNode OutputNode)
					Loop = await OutputNode.GetParent();
				else
					Loop = Loop.Parent;
			}
		...
		internal async Task<OutputNode> LoadParent()
		{
			...
			this.parent = await Database.LoadObject<OutputNode>(this.parentId);
			OutputSource.RegisterNode(this.parent);
		...
		{
			return Task.FromResult(Caller.HasPrivilege("Source." + OutputSource.SourceID + ".Node.View"));
		}
		...
		{
			return Task.FromResult(Caller.HasPrivilege("Source." + OutputSource.SourceID + ".Node.Edit"));
		}
		...
		{
			return Task.FromResult(Caller.HasPrivilege("Source." + OutputSource.SourceID + ".Node.Add"));
		}
		...
		{
			return Task.FromResult(Caller.HasPrivilege("Source." + OutputSource.SourceID + ".Node.Destroy"));
		}
		...
		{
			Namespace Namespace = await Language.GetNamespaceAsync(typeof(OutputNode).Namespace)
				?? await Language.CreateNamespaceAsync(typeof(OutputNode).Namespace);

			LinkedList<Parameter> Result = new LinkedList<Parameter>();
			Result.AddLast(new StringParameter("NodeId", await Namespace.GetStringAsync(1, "Output ID"), this.nodeId));
		...
		{
			IEnumerable<OutputMessage> Messages = await Database.Find<OutputMessage>(
				new FilterFieldEqualTo("NodeId", this.objectId), "Created");
			LinkedList<Message> Result = new LinkedList<Message>();

			foreach (OutputMessage Msg in Messages)
		...
		{
			if (!(await this.GetParent() is OutputNode Parent))
				return false;
			else
		...
		{
			if (!(Child is OutputNode Node))
				throw new Exception("Child must be an output node.");
			...
			if (Node.objectId == Guid.Empty)
			{
				...
				OutputSource.RegisterNode(Node);
				...
				await OutputSource.NewEvent(Event);
			}
			else
		...
		protected virtual async Task NodeUpdated()
		{
			...
			await OutputSource.NewEvent(new NodeUpdated()
			{
			...
			{
				OutputSource.RegisterNewNodeId(this, this.oldId);
		...
		public virtual async Task<bool> RemoveAsync(INode Child)
		{
			...
			if (Node.objectId != Guid.Empty)
			{
				...
				await OutputSource.NewEvent(new NodeRemoved()
				{
					NodeId = Node.NodeId,
					Partition = Node.Partition,
					SourceId = Node.SourceId,
		...
		public async virtual Task DestroyAsync()
		{
			...
			OutputSource.UnregisterNode(this);
		}
