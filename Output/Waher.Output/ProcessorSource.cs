...
{
	/// <summary>
	/// Defines the Output data source. This data source contains a tree structure of 
	/// output of nodes
	/// </summary>
	public class OutputSource : IDataSource
	{
		/// <summary>
		/// Source ID for the output data source.
		/// </summary>
		public const string SourceID = "Output";

		private static readonly Dictionary<string, OutputNode> nodes = new Dictionary<string, OutputNode>();
		private static Root root = null;
		private static OutputSource instance = null;
		...
		public DateTime LastChanged
		{
			...
			internal set
			{
				if (this.lastChanged != value)
				{
					...
					Task _ = RuntimeSettings.SetAsync("OutputSource.LastChanged", value);
				}
			}
		}
		...
		{
			return Language.GetStringAsync(typeof(OutputSource), 13, "Output");
		}
		...
		/// <returns>Node, if found, null otherwise.</returns>
		public static Task<OutputNode> GetNode(string NodeId)
		{
		...
		/// <param name="NodeRef">Node reference</param>
		/// <returns>Node, if found, null otherwise.</returns>
		public static async Task<OutputNode> GetNode(IThingReference NodeRef)
		{
			...
			lock (nodes)
			{
				if (nodes.TryGetValue(NodeRef.NodeId, out OutputNode Node))
					return Node;
			}

			foreach (OutputNode Node2 in await Database.Find<OutputNode>(new FilterFieldEqualTo("NodeId", NodeRef.NodeId)))
			{
				lock (nodes)
				{
					if (nodes.TryGetValue(NodeRef.NodeId, out OutputNode Node))
		...
		internal static OutputNode RegisterNode(OutputNode Node)
		{
			if (Node.SourceId == SourceID && string.IsNullOrEmpty(Node.Partition))
			{
				lock (nodes)
				{
					if (nodes.TryGetValue(Node.NodeId, out OutputNode Node2))
						return Node2;
					else
					{
		...
		internal static OutputNode RegisterNewNodeId(OutputNode Node, string OldId)
		{
			if (Node.SourceId != SourceID || !string.IsNullOrEmpty(Node.Partition))
				return Node;
			...
			lock (nodes)
			{
				if (!nodes.TryGetValue(OldId, out OutputNode Node2) || Node2 != Node)
					return Node;
		...
		internal static void UnregisterNode(OutputNode Node)
		{
			if (Node.SourceId == SourceID && string.IsNullOrEmpty(Node.Partition))
			{
				lock (nodes)
				{
					if (nodes.TryGetValue(Node.NodeId, out OutputNode Node2) && Node == Node2)
		...
		private static async Task LoadRoot()
		{
			...
			foreach (OutputNode Node in await Database.Find<OutputNode>(new FilterFieldEqualTo("ParentId", Guid.Empty)))
			{
			...
			if (Result is null)
			{
				Result = new Root()
				{
					NodeId = await (await Translator.GetDefaultLanguageAsync()).GetStringAsync(typeof(OutputSource), 5, "Root")
				};
		...
		public static async Task<int> DeleteOldEvents(TimeSpan MaxAge)
		{
			...
			if (NrEvents > 0)
			{
				...
				if (NrEvents == 1)
					Log.Informational("Deleting 1 output event from the database.", SourceID, Tags);
				else
					Log.Informational("Deleting " + NrEvents.ToString() + " output events from the database.", SourceID, Tags);
			}
		...
		public static async Task<int> DeleteOrphans()
		{
			...
			foreach (OutputNode Node in await Database.Find<OutputNode>())
			{
				...
				OutputNode ParentNode = await Database.TryLoadObject<OutputNode>(Node.ParentId);
