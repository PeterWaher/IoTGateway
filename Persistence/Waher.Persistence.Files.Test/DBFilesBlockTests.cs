...
#endif
{
	...
	public class BlocksTests
	{
		...
		private async Task ParseBlock(byte[] Block)
		{ 
			BinaryDeserializer Reader = new BinaryDeserializer("Default", Encoding.UTF8, Block, int.MaxValue);
			GenericObjectSerializer Serializer = new GenericObjectSerializer(provider, false);
			...
			Console.Out.WriteLine("BytesUsed: " + BytesUsed);
			Console.Out.WriteLine("SizeSubtree: " + SizeSubtree);
			Console.Out.WriteLine("LastBlockIndex: " + LastBlockIndex);
			Console.Out.WriteLine("ParentBlockIndex: " + ParentBlockIndex);

			while (Reader.Position < BytesUsed)
			{
				Console.Out.WriteLine();
				Console.Out.WriteLine("Position: " + Reader.Position);

				uint LeftLink = Reader.ReadUInt32();

				Console.Out.WriteLine("LeftLink: " + LeftLink);

				GenericObject Obj = await Serializer.Deserialize(Reader, ObjectSerializer.TYPE_OBJECT, false, false) as GenericObject;
				Assert.IsNotNull(Obj);

				Console.Out.WriteLine("CollectionName: " + Obj.CollectionName);
				Console.Out.WriteLine("TypeName: " + Obj.TypeName);
				Console.Out.WriteLine("ObjectId: " + Obj.ObjectId);
				Console.Out.WriteLine("ArchivingTime: " + Obj.ArchivingTime);
