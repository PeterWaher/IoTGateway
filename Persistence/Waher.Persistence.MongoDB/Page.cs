...
namespace Waher.Persistence.MongoDB
{
		...
		where T : class
	{
		...
		public Page(int PageSize, string Collection, Filter Filter, string[] SortOrder,
			IEnumerable<T> Items, ObjectSerializer Serializer, MongoDBProvider Provider)
		{
		...
		public async Task<IPage<T>> FindNext()
		{
			...
			ObjectId LastObjectId = await this.serializer.GetObjectId(this.lastItem, false);
			if (LastObjectId == ObjectId.Empty)
				return new EmptyPage<T>();
