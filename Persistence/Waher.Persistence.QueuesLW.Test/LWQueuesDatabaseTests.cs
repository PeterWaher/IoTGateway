...
{
	[TestClass]
	public sealed class DBQueuesDatabaseTests
	{
		internal const string DataFolder = "Data";
		internal const string UmlFolder = "UmlDatabase";
...
#if LW
			provider = await FilesProvider.CreateAsync(DataFolder, CollectionName, 8192, 8192, 4096, Encoding.UTF8, 8192);
#else
			provider = await FilesProvider.CreateAsync(DataFolder, CollectionName, 8192, 8192, 4096, Encoding.UTF8, 8192, true);
#endif
		...
		{	
			...
			this.queue = new DatabaseQueue("UnitTest", this.profiler);
		}
		...
		public async Task TestCleanup()
		{
			...
			Assert.IsNull(Item, "Queue not cleared.");
		}
		...
		public async Task Test_13_DequeueEnqueue_MultipleThreads_ValueTypes()
		{
			...
			for (int i = 0; i < 10; i++)
			{
				Enqueuers[i] = new(i * 100, 100, 0, 1, Rnd, this.queue);
				_ = Task.Run(Enqueuers[i].Start, CancellationToken.None);
			}

			for (int i = 0; i < 10; i++)
			{
				Dequeuers[i] = new(100, 0, 1, Rnd, this.queue, Dequeued);
