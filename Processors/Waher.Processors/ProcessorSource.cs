...
{
	/// <summary>
	/// Defines the Processors data source. This data source contains a tree structure of 
	/// processor of nodes
	/// </summary>
	public class ProcessorSource : IDataSource
	{
		/// <summary>
		/// Source ID for the processors data source.
		/// </summary>
		public const string SourceID = "Processors";

		private static readonly Dictionary<string, ProcessorNode> nodes = new Dictionary<string, ProcessorNode>();
		private static Root root = null;
		private static ProcessorSource instance = null;
		...
		public DateTime LastChanged
		{
			...
			internal set
			{
				if (this.lastChanged != value)
				{
					...
					Task _ = RuntimeSettings.SetAsync("ProcessorSource.LastChanged", value);
				}
			}
		}
		...
		{
			return Language.GetStringAsync(typeof(ProcessorSource), 13, "Processors");
		}
		...
		/// <returns>Node, if found, null otherwise.</returns>
		public static Task<ProcessorNode> GetNode(string NodeId)
		{
		...
		/// <param name="NodeRef">Node reference</param>
		/// <returns>Node, if found, null otherwise.</returns>
		public static async Task<ProcessorNode> GetNode(IThingReference NodeRef)
		{
			...
			lock (nodes)
			{
				if (nodes.TryGetValue(NodeRef.NodeId, out ProcessorNode Node))
					return Node;
			}

			foreach (ProcessorNode Node2 in await Database.Find<ProcessorNode>(new FilterFieldEqualTo("NodeId", NodeRef.NodeId)))
			{
				lock (nodes)
				{
					if (nodes.TryGetValue(NodeRef.NodeId, out ProcessorNode Node))
		...
		internal static ProcessorNode RegisterNode(ProcessorNode Node)
		{
			if (Node.SourceId == SourceID && string.IsNullOrEmpty(Node.Partition))
			{
				lock (nodes)
				{
					if (nodes.TryGetValue(Node.NodeId, out ProcessorNode Node2))
						return Node2;
					else
					{
		...
		internal static ProcessorNode RegisterNewNodeId(ProcessorNode Node, string OldId)
		{
			if (Node.SourceId != SourceID || !string.IsNullOrEmpty(Node.Partition))
				return Node;
			...
			lock (nodes)
			{
				if (!nodes.TryGetValue(OldId, out ProcessorNode Node2) || Node2 != Node)
					return Node;
		...
		internal static void UnregisterNode(ProcessorNode Node)
		{
			if (Node.SourceId == SourceID && string.IsNullOrEmpty(Node.Partition))
			{
				lock (nodes)
				{
					if (nodes.TryGetValue(Node.NodeId, out ProcessorNode Node2) && Node == Node2)
		...
		private static async Task LoadRoot()
		{
			...
			foreach (ProcessorNode Node in await Database.Find<ProcessorNode>(new FilterFieldEqualTo("ParentId", Guid.Empty)))
			{
		...
		public static async Task<int> DeleteOldEvents(TimeSpan MaxAge)
		{
			...
			if (NrEvents > 0)
			{
				...
				if (NrEvents == 1)
					Log.Informational("Deleting 1 processor event from the database.", SourceID, Tags);
				else
					Log.Informational("Deleting " + NrEvents.ToString() + " processor events from the database.", SourceID, Tags);
			}
		...
		public static async Task<int> DeleteOrphans()
		{
			...
			foreach (ProcessorNode Node in await Database.Find<ProcessorNode>())
			{
				...
				ProcessorNode ParentNode = await Database.TryLoadObject<ProcessorNode>(Node.ParentId);
