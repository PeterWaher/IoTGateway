...
namespace Waher.Reports.Files.Model.Parameters
{
	...
	public class DateParameter : ReportParameterWithOptions
	{
		...
		public DateParameter(XmlElement Xml)
			: base(Xml)
		{
		...
		public override async Task PopulateForm(DataForm Parameters, Language Language, Variables Variables)
		{
			...
			if (Default.HasValue)
				DefaultValue = new string[] { XML.Encode(Default.Value, true) };
			else
			...
			if (Min.HasValue || Max.HasValue)
			{
				Validation = new RangeValidation(
					Min.HasValue ? XML.Encode(Min.Value, true) : null,
					Max.HasValue ? XML.Encode(Max.Value, true) : null);
			}
			else
			...
			if (Attributes.RestrictToOptions)
			{
				Field = new ListSingleField(Parameters, Attributes.Name, Attributes.Label, Attributes.Required,
					DefaultValue, Attributes.Options, Attributes.Description, DateDataType.Instance, Validation, 
			...
			SetEditableFormResult Result)
		{
			...
			else
			{
				...
				{
					Variables[Name] = Parsed.Date;

					if (Parsed.TimeOfDay != TimeSpan.Zero)
						Result.AddError(Name, await Language.GetStringAsync(typeof(ReportFileNode), 3, "Only date acceptable."));
				}
				else
