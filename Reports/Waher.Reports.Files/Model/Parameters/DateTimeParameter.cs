...
{
    /// <summary>
    /// Represents a Date and Time-valued parameter.
    /// </summary>
    public class DateTimeParameter : ReportParameterWithOptions
	{
		...
		public DateTimeParameter(string Page, string Name, string Label, string Description,
        ...
        public override Task PopulateForm(DataForm Parameters, Language Language, object Value)
        {
            ...
            if (this.RestrictToOptions)
            {
                Field = new ListSingleField(Parameters, this.Name, this.Label, this.Required,
                    new string[] { this.DefaultValue.HasValue ? XML.Encode(this.DefaultValue.Value) : string.Empty },
                    this.GetOptionTags(), this.Description, DateTimeDataType.Instance, Validation, string.Empty, false, false, false);
            }
            else
            {
