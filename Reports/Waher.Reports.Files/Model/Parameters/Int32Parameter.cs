...
namespace Waher.Reports.Files.Model.Parameters
{
	...
	/// </summary>
	public class Int32Parameter : ReportParameterWithOptions
	{
		private readonly ReportInt32Attribute defaultValue;
		private readonly ReportInt32Attribute min;
		private readonly ReportInt32Attribute max;

		/// <summary>
		/// Represents a Int32-valued parameter.
		/// </summary>
		/// <param name="Xml">XML definition.</param>
		public Int32Parameter(XmlElement Xml)
			: base(Xml)
		{
			this.defaultValue = new ReportInt32Attribute(Xml, "default");
			this.min = new ReportInt32Attribute(Xml, "min");
			this.max = new ReportInt32Attribute(Xml, "max");
		}
		...
		public override async Task PopulateForm(DataForm Parameters, Language Language, Variables Variables)
		{
			...
			int? Default = this.defaultValue.IsEmpty ? null : (int?)await this.defaultValue.Evaluate(Variables);
			int? Min = this.min.IsEmpty ? null : (int?)await this.min.Evaluate(Variables);
			int? Max = this.max.IsEmpty ? null : (int?)await this.max.Evaluate(Variables);
			...
			if (Attributes.RestrictToOptions)
			{
				Field = new ListSingleField(Parameters, Attributes.Name, Attributes.Label, Attributes.Required,
					DefaultValue, Attributes.Options, Attributes.Description, IntDataType.Instance, Validation,
			...
			SetEditableFormResult Result)
		{
			...
			else
			{
				...
				else if (int.TryParse(s, out int Parsed))
