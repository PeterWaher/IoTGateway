...
namespace Waher.Runtime.Collections.Test
{
	...
	public class ChunkedListBenchmarkingTests
	{
		...
		[TestMethod]
		public Task Test_23_LastIndexOf_Small()
		{
			return Test_LastIndexOf("Test_23_LastIndexOf_Small", smallNumberOfItems, 500);
		}

		[TestMethod]
		public Task Test_24_LastIndexOf_Large()
		{
			return Test_LastIndexOf("Test_24_LastIndexOf_Large", largeNumberOfItems, 2000);
		}

		private static async Task Test_LastIndexOf(string Name, int[] NumberOfItems, int Limit)
		{
			Benchmarker2D Benchmarker = new();
			LinkedList<double> LinkedList;
			List<double> List;
			...
			lock (syncObj)
			{
				foreach (int N in NumberOfItems)
				{
					...
					using (Benchmarking Test = Benchmarker.Start("LinkedList", N))
					{
						foreach (double Item in Items)
						{
							if (LinkedList.FindLast(Item) is null)
								throw new Exception("Item not found in LinkedList");
						}
					}
					...
					using (Benchmarking Test = Benchmarker.Start("ChunkedList", N))
					{
						foreach (double Item in Items)
						{
							if (ChunkedList.LastIndexOf(Item) < 0)
								throw new Exception("Item not found in ChunkedList");
						}
					}
				}
