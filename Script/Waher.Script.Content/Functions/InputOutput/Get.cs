	...
	public class Get : FunctionMultiVariate
	{
		...
		public override async Task<IElement> EvaluateAsync(IElement[] Arguments, Variables Variables)
		{
			...
			if (Arguments.Length > 2)
			{
				...
				Content = await InternetContent.GetAsync(Url, Certificate, HeaderList?.ToArray() ?? Array.Empty<KeyValuePair<string, string>>());
			}
			else
				Content = await InternetContent.GetAsync(Url, HeaderList?.ToArray() ?? Array.Empty<KeyValuePair<string, string>>());
		...
		internal static ChunkedList<KeyValuePair<string, string>> GetHeaders(object Arg, ScriptNode Node)
		{
			...
			if (Arg is IDictionary<string, IElement> Headers)
			{
				...
				foreach (KeyValuePair<string, IElement> P in Headers)
					HeaderList.Add(new KeyValuePair<string, string>(P.Key, ToString(P.Value)));
			}
