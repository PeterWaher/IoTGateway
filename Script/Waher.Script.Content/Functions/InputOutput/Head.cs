...
namespace Waher.Script.Content.Functions.InputOutput
{
	...
	public class Head : FunctionMultiVariate
	{
		...
		/// <param name="Length">Length of expression covered by node.</param>
		/// <param name="Expression">Expression containing script.</param>
		public Head(ScriptNode Url, ScriptNode Headers, ScriptNode Certificate, int Start, int Length, Expression Expression)
		...
		public override async Task<IElement> EvaluateAsync(IElement[] Arguments, Variables Variables)
		{
			...
			if (Arguments.Length > 2)
			{
				...
				Content = await InternetContent.HeadAsync(Url, Certificate, HeaderList?.ToArray() ?? Array.Empty<KeyValuePair<string, string>>());
			}
			else
				Content = await InternetContent.HeadAsync(Url, HeaderList?.ToArray() ?? Array.Empty<KeyValuePair<string, string>>());
