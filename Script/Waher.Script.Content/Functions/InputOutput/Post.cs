	...
	public class Post : FunctionMultiVariate
	{
		...
		public override async Task<IElement> EvaluateAsync(IElement[] Arguments, Variables Variables)
		{
			...
			if (Arguments.Length > 3)
			{
				...
				Content = await InternetContent.PostAsync(Url, Data, Certificate, HeaderList?.ToArray() ?? Array.Empty<KeyValuePair<string, string>>());
			}
			else
				Content = await InternetContent.PostAsync(Url, Data, HeaderList?.ToArray() ?? Array.Empty<KeyValuePair<string, string>>());
