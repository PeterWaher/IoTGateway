...
namespace Waher.Script.Fractals.ColorModels
{
    ...
    /// 
    /// RandomLinearComplementaryHSL()                  N = 1024 by default
    /// RandomLinearComplementaryHSL(N)                 BandSize=16 by default
    /// RandomLinearComplementaryHSL(N,BandSize)        Seed=random by default.
    ...
    /// </example>
    public class RandomLinearComplementaryHSL : FunctionMultiVariate
    {
		public RandomLinearComplementaryHSL(ScriptNode N, int Start, int Length, Expression Expression)
			: base(new ScriptNode[] { N }, argumentTypes1Scalar, Start, Length, Expression)
		{
		}

		public RandomLinearComplementaryHSL(ScriptNode N, ScriptNode BandSize, int Start, int Length, Expression Expression)
			: base(new ScriptNode[] { N, BandSize }, argumentTypes2Scalar, Start, Length, Expression)
		{
		}

		public RandomLinearComplementaryHSL(ScriptNode N, ScriptNode BandSize, ScriptNode Seed, int Start, int Length, Expression Expression)
        ...
        {
            if (N <= 0)
                throw new ScriptRuntimeException("N in RandomLinearComplementaryHSL(N[,BandSize]) has to be positive.", Node);

            if (BandSize <= 0)
                throw new ScriptRuntimeException("BandSize in RandomLinearComplementaryHSL(N[,BandSize]) has to be positive.", Node);
            ...
            lock (Generator)
            {
                ...
                while (i < N)
                {
                    ...
                    H += Generator.NextDouble() * 120 + 120;
                    S = Generator.NextDouble();
                    L = Generator.NextDouble();
