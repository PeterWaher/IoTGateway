...
namespace Waher.Script.Fractals.ColorModels
{
    ...
    /// 
    /// RandomLinearComplementaryHSV()                  N = 1024 by default
    /// RandomLinearComplementaryHSV(N)                 BandSize=16 by default
    /// RandomLinearComplementaryHSV(N,BandSize)        Seed=random by default.
    /// RandomLinearComplementaryHSV(N,BandSize,Seed)
    /// </summary>
    /// <example>
    /// testcolormodel(RandomLinearComplementaryHSV(1024,16))
    /// MandelbrotFractal(-0.72900390625,-0.2434140625,0.046875,randomlinearcomplementaryhsv(1024,16,3),1024,768)
    /// </example>
    public class RandomLinearComplementaryHSV : FunctionMultiVariate
    {
        ...
        public static SKColor[] CreatePalette(int N, int BandSize, int? Seed, ScriptNode Node)
        {
            ...
            if (BandSize <= 0)
                throw new ScriptRuntimeException("BandSize in RandomLinearComplementaryHSV(N[,BandSize]) has to be positive.", Node);
        ...
        {
            get { return "RandomLinearComplementaryHSV"; }
        }
    }
}
