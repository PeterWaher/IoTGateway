...
namespace Waher.Script.Fractals.ComplexFractals
{
	...
	/// </remarks>
	/// <example>
	/// NewtonTopographyFractal((0,0),3,,[-1,0,0,1])
	/// NewtonTopographyFractal((0,0),3,,[-1,0,1])
	/// NewtonTopographyFractal((0,0),3,,[-1,0,0,0,1])
	/// NewtonTopographyFractal((0,0),3,,[-1,0,0,0,0,1])
	/// NewtonTopographyFractal((0,0),3,,[-1,0,0,0,0,0,1])
	/// NewtonTopographyFractal((0,0),3,,[-1,0,0,0,0,0,0,1])
	/// NewtonTopographyFractal((0,0),5,,[2,-2,0,1])
	/// NewtonTopographyFractal((0,0),5,,[-16,0,0,0,15,0,0,0,1])
	/// NewtonTopographyFractal((0,0),10,,[-16,0,0,0,15,0,0,0,1])
	/// NewtonTopographyFractal((-0.0732421875,0.0557911132812496),0.0029296875,,[1, 0, 0, 4, 0, 0, 0, 15],,640,480)
	/// NewtonTopographyFractal((0,0),3,,Uniform(0,5,8),,640,480)
	/// NewtonTopographyFractal((0,0),3,,Uniform(0,5,8),RandomLinearRGB(1024,4,1325528060), 640,480)
	/// 
	/// Generalized Newton Fractals:
	/// NewtonTopographyFractal((0,0),3,0.5,[-1,0,0,1],,640,480)
	/// NewtonTopographyFractal((0,0),3,1.5,[-1,0,0,1],,640,480)
	/// NewtonTopographyFractal((0,0),3,0.5,Uniform(0,5,8),,640,480)
	/// NewtonTopographyFractal((0,0),3,1.5,Uniform(0,5,8),,640,480)
	/// 
	/// Using Lambda expressions:
	/// NewtonTopographyFractal((pi/2,0),pi/4,,x->sin(x),RandomLinearRGB(128,16,1177860657),640,480)
	/// </example>
	public class NewtonTopographyFractal : FunctionMultiVariate
	{
		...
		public NewtonTopographyFractal(ScriptNode z, ScriptNode dr, ScriptNode R, ScriptNode c,
			int Start, int Length, Expression Expression)
		...
		public override IElement Evaluate(IElement[] Arguments, Variables Variables)
		{
			...
			if (i >= c)
				throw new ScriptRuntimeException("Insufficient parameters in call to NewtonTopographyFractal().", this);
			...
			if (i < c)
			{
				...
				else if (Arguments[i].AssociatedObjectValue is ILambdaExpression)
				{
					...
					if (f.NrArguments != 1)
						throw new ScriptRuntimeException("Lambda expression in calls to NewtonTopographyFractal() must be of one variable.", this);
				...
				else
				{
					throw new ScriptRuntimeException("Parameter " + (i + 1).ToString() +
						" in call to NewtonTopographyFractal has to be a vector of numbers, containing coefficients " +
			...
			if (i < c)
			{
				throw new ScriptRuntimeException("Parameter mismatch in call to NewtonTopographyFractal(r,c,dr,Coefficients[,Palette][,dimx[,dimy]]).",
					this);
			}
			...
			if (!(f is null))
			{
				return CalcNewtonTopography(rc, ic, dr, R, f, fDef, Variables, Palette, dimx, dimy,
		...
		private string FractalZoomScript(double r, double i, double Size, object State)
		{
			...
			sb.Append("NewtonTopographyFractal(");
		...
		{
			get { return "NewtonTopographyFractal"; }
		}
	}
}
