...
namespace Waher.Script.Persistence.Functions
{
	...
	/// </summary>
	public class VerifyHash : FunctionMultiVariate
	{
		/// <summary>
		/// Verifies a hash value
		/// </summary>
		...
		public VerifyHash(ScriptNode Hash, int Start, int Length, Expression Expression)
		...
		/// <param name="Length">Length of expression covered by node.</param>
		/// <param name="Expression">Expression containing script.</param>
		public VerifyHash(ScriptNode Realm, ScriptNode Hash, int Start, int Length, Expression Expression)
		...
		/// </summary>
		public override string FunctionName => nameof(VerifyHash);
		...
		public override async Task<IElement> EvaluateAsync(IElement[] Arguments, Variables Variables)
		{
			...
			if (Arguments.Length > 1)
			{
				...
				Result = await PersistedHashes.VerifyHash(Realm, Hash);
			}
			else
				Result = await PersistedHashes.VerifyHash(Hash);
