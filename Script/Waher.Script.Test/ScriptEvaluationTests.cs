...
namespace Waher.Script.Test
{
	...
	public class ScriptEvaluationTests
	{
		...
		public async Task Evaluation_Test_39_Runtime()
		{
			...
			await Test("Obj:={a:1,b:2};exists(x:=Obj['b'])", true);
			await Test("Obj:={a:1,b:2};exists(Obj['c'])", false);
			await Test("Obj:={a:1,b:2};exists(Obj2['a'])", false);
			await Test("Obj:={a:1,b:2,c:null};exists(Obj2['c'])", false);
			await Test("Obj:={a:1,b:2,c:null};exists(x:=Obj2['c'])", false);
			...
			await Test("Obj:=[1,2];exists(x:=Obj[1])", true);
			await Test("Obj:=[1,2];exists(Obj[2])", false);
			await Test("Obj:=[1,2,null];exists(Obj[2])", false);
			await Test("Obj:=[1,2,null];exists(x:=Obj[2])", false);
