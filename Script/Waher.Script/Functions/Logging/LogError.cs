...
namespace Waher.Script.Functions.Logging
{
	...
	/// </summary>
	public class LogError : LogFunction
	{
		/// <summary>
		/// Logs an error event to the event log.
		/// </summary>
		...
		public LogError(ScriptNode Message, int Start, int Length, Expression Expression)
		...
		/// <param name="Length">Length of expression covered by node.</param>
		/// <param name="Expression">Expression containing script.</param>
		public LogError(ScriptNode Message, ScriptNode Tags, int Start, int Length, Expression Expression)
		...
		/// </summary>
		public override string FunctionName => nameof(LogError);
		...
		{
			Log.Error(Message, Object, Actor, EventId, Level, Facility, Module, StackTrace, Tags);
		}
	}
}
