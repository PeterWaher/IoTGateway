	...
	public abstract class QuaternaryOperator : TernaryOperator
	{
		...
		public override bool ForAllChildNodes(ScriptNodeEventHandler Callback, object State, SearchMethod Order)
		{
			if (Order == SearchMethod.DepthFirst)
			{
				...
				if (!(this.middle2?.ForAllChildNodes(Callback, State, Order) ?? true))
					return false;
			...
			if (!(this.middle2 is null))
			{
				...
				if (b || (Order == SearchMethod.TreeOrder && !this.middle2.ForAllChildNodes(Callback, State, Order)))
				{
					if (RecalcIsAsync)
						this.CalcIsAsync();
