...
namespace Waher.Script.Objects.Matrices
{
	...
	public sealed class ObjectMatrix : RingElement, IMatrix
	{
		...
		{
			return this.TryFind(Element.AssociatedObjectValue, FromColumn, FromRow, out Column, out Row);
		}
		...
		/// <param name="Row">Row, if found.</param>
		/// <returns>If element was found.</returns>
		public bool TryFind(object Element, int FromColumn, int FromRow, out int Column, out int Row)
		{
			...
			while (FromRow < this.rows)
			{
				...
				{
					Item = Values[FromRow, FromColumn];

					if (Item?.Equals(Element) ?? (Element is null))
					{
						Column = FromColumn;
						Row = FromRow;
		...
		{
			return this.TryFindLast(Element.AssociatedObjectValue, FromColumn, FromRow, out Column, out Row);
		}
		...
		/// <param name="Row">Row, if found.</param>
		/// <returns>If element was found.</returns>
		public bool TryFindLast(object Element, int FromColumn, int FromRow, out int Column, out int Row)
		{
			object[,] Values = this.Values;
			object Item;
