...
{
	/// <summary>
	/// Converts double numbers to 16-bit integer values.
	/// </summary>
	public class DoubleToInt16 : ITypeConverter
	{
		/// <summary>
		/// Converts double numbers to 16-bit integer values.
		/// </summary>
		...
		{
			if (Value is double d && d >= short.MinValue && d <= short.MaxValue && Math.Round(d) == d)
				return (short)d;
			else
				throw new ArgumentException("Expected 16-bit integer value.", nameof(Value));
		}
		...
		{
			if (Value is double d && d >= short.MinValue && d <= short.MaxValue && Math.Round(d) == d)
				return new ObjectValue((short)d);
			else
				throw new ArgumentException("Expected 16-bit integer value.", nameof(Value));
		}
	}
