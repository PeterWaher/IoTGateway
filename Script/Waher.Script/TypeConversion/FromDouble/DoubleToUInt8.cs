...
{
	/// <summary>
	/// Converts double numbers to 8-bit unsigned integer values.
	/// </summary>
	public class DoubleToUInt8 : ITypeConverter
	{
		/// <summary>
		/// Converts double numbers to 8-bit unsigned integer values.
		/// </summary>
		...
		{
			if (Value is double d && d >= byte.MinValue && d <= byte.MaxValue && Math.Round(d) == d)
				return (byte)d;
			else 
				throw new ArgumentException("Expected 8-bit unsigned integer value.", nameof(Value));
		}
