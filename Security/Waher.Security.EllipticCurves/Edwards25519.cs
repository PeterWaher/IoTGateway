...
namespace Waher.Security.EllipticCurves
{
    ...
    public class Edwards25519 : EdwardsTwistedCurve
    {
        ...
        public override Tuple<byte[], byte[]> CalculatePrivateKey(byte[] Secret)
        {
            ...
            if (this.hashSecret)
				Buffer.BlockCopy(Bin, 0, PrivateKey, 0, 32);
            else
				Buffer.BlockCopy(Secret, 0, PrivateKey, 0, Math.Min(32, Secret.Length));

			Buffer.BlockCopy(Bin, 32, AdditionalInfo, 0, 32);
        ...
        {
            return EdDSA.Sign(Data, BigEndian, this.PrivateKey, this.AdditionalInfo, Hashes.ComputeSHA512Hash, this);
        }
        ...
        {
            return EdDSA.Verify(Data, PublicKey, BigEndian, Hashes.ComputeSHA512Hash, this, Signature);
        }
