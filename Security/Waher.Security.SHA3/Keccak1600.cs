    ...
    public abstract class Keccak1600
    {
        ...
        public byte[] ComputeFixed(byte[] S)
        {
            ...
            for (ir = 0; ir < 24; ir++)
            {
                ...
                A[0] = A2[0] ^ (A2[2] & ~A2[1]);
                A[5] = A2[5] ^ (A2[7] & ~A2[6]);
                A[10] = A2[10] ^ (A2[12] & ~A2[11]);
                A[15] = A2[15] ^ (A2[17] & ~A2[16]);
                A[20] = A2[20] ^ (A2[22] & ~A2[21]);

                A[1] = A2[1] ^ (A2[3] & ~A2[2]);
                A[6] = A2[6] ^ (A2[8] & ~A2[7]);
                A[11] = A2[11] ^ (A2[13] & ~A2[12]);
                A[16] = A2[16] ^ (A2[18] & ~A2[17]);
                A[21] = A2[21] ^ (A2[23] & ~A2[22]);

                A[2] = A2[2] ^ (A2[4] & ~A2[3]);
                A[7] = A2[7] ^ (A2[9] & ~A2[8]);
                A[12] = A2[12] ^ (A2[14] & ~A2[13]);
                A[17] = A2[17] ^ (A2[19] & ~A2[18]);
                A[22] = A2[22] ^ (A2[24] & ~A2[23]);

                A[3] = A2[3] ^ (A2[0] & ~A2[4]);
                A[8] = A2[8] ^ (A2[5] & ~A2[9]);
                A[13] = A2[13] ^ (A2[10] & ~A2[14]);
                A[18] = A2[18] ^ (A2[15] & ~A2[19]);
                A[23] = A2[23] ^ (A2[20] & ~A2[24]);

                A[4] = A2[4] ^ (A2[1] & ~A2[0]);
                A[9] = A2[9] ^ (A2[6] & ~A2[5]);
                A[14] = A2[14] ^ (A2[11] & ~A2[10]);
                A[19] = A2[19] ^ (A2[16] & ~A2[15]);
                A[24] = A2[24] ^ (A2[21] & ~A2[20]);
