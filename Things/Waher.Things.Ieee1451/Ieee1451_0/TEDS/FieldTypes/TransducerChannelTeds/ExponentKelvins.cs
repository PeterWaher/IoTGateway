...
{
	/// <summary>
	/// TEDS The exponent for kelvins (§6.5.2.14)
	/// </summary>
	public class ExponentKelvins : TedsRecord
    {
		/// <summary>
		/// TEDS The exponent for kelvins (§6.5.2.14)
		/// </summary>
		public ExponentKelvins()
            : base()
        {
        }
        ...
        {
            return RecordTypeId.Class == 3 && RecordTypeId.Type == 57 ? Grade.Perfect : Grade.NotAtAll;
        }
        ...
        {
			State.Units.Kelvins = RawValue.NextUInt8(nameof(ExponentKelvins));

            return new ExponentKelvins()
            {
				Class = RecordTypeId.Class,
				Type = RecordTypeId.Type,
				RawValue = RawValue.Body,
				Exponent = State.Units.Kelvins
            };
        }
		...
		{
			SnifferOutput.Append("ExponentKelvins=");
