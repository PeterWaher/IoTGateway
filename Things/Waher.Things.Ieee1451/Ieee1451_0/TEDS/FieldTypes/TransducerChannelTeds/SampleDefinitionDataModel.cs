...
namespace Waher.Things.Ieee1451.Ieee1451_0.TEDS.FieldTypes.TransducerChannelTeds
{
	...
	public class SampleDefinitionDataModel : TedsRecord
	{
		...
		{
			State.SampleDefinition.DataModel = RawValue.NextUInt8<Ieee1451_0SampleDefinitionDataModel>(null);

			if (RawValue.HasSniffers)
				RawValue.SniffValue(nameof(this.DataModel), State.SampleDefinition.DataModel.ToString());
		...
		{
			SnifferOutput.Append("DataModel=");
			SnifferOutput.AppendLine(this.DataModel.ToString());
		}
	}
}
