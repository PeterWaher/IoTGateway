...
{
	/// <summary>
	/// TEDS Design operational lower range limit (§6.5.2.19)
	/// </summary>
	public class UpperLimit : TedsRecord
	{
		/// <summary>
		/// TEDS Design operational lower range limit (§6.5.2.19)
		/// </summary>
		...
		/// <summary>
		/// For sensors, this shall be the highest valid value for TransducerChannel 
		/// data that the TransducerChannel is designed to provide after correction is 
		/// applied.
		/// </summary>
		...
		{
			return RecordTypeId.Class == 3 && RecordTypeId.Type == 14 ? Grade.Perfect : Grade.NotAtAll;
		}
		...
		public override TedsRecord Parse(ClassTypePair RecordTypeId, Binary RawValue, ParsingState State)
		{
			return new UpperLimit()
			{
				...
				Value = RawValue.NextSingle(nameof(UpperLimit))
			};
		}
		...
		{
			Fields.Add(new QuantityField(Thing, Timestamp, (Teds?.FieldName ?? "Value") + ", Upper Limit", this.Value,
		...
		{
			SnifferOutput.Append("UpperLimit=");
