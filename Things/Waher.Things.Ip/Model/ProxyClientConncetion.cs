...
namespace Waher.Things.Ip.Model
{
    ...
    public class ProxyClientConncetion : CommunicationLayer, IDisposable
	{
		...
		private async Task<bool> Outgoing_OnReceived(object Sender, bool ConstantBuffer, byte[] Buffer, int Offset, int Count)
		{
			if (await this.incoming.SendAsync(ConstantBuffer, Buffer, Offset, Count))
			{
		...
		private async Task<bool> Incoming_OnReceived(object Sender, bool ConstantBuffer, byte[] Buffer, int Offset, int Count)
		{
			if (await this.outgoing.SendAsync(ConstantBuffer, Buffer, Offset, Count))
			{
