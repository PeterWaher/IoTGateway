...
namespace Waher.Things.Jobs.Commands
{
	/// <summary>
	...
	public class ClearMessages : ICommand
	{
		...
		public ClearMessages(JobNode Node)
		{
			this.node = Node;
		}
		...
		{
			IEnumerable<JobMessage> Messages = await Database.FindDelete<JobMessage>(
		...
		{
			return Language.GetStringAsync(typeof(JobSource), 6, "Erase messages");
		}
		...
		{
			return Language.GetStringAsync(typeof(JobSource), 7, "Messages successfully erased.");
		}
		...
		{
			return Language.GetStringAsync(typeof(JobSource), 8, "Are you sure you want to erase all messages for the selected job(s)?");
		}
		...
		{
			return Language.GetStringAsync(typeof(JobSource), 9, "Unable to erase messages.");
		}
