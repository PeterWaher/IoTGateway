...
namespace Waher.Utility.RegEx
{
	class Program
	{
		...
		/// Waher.Utility.RegEx.exe -p "C:\My Projects\IoTGateway\*.csproj" -p "C:\My Projects\IoTGateway\*.cs" -p "C:\My Projects\IoTGateway\*.nuspec" -f "Copyright (©|\([cC]\)) (?'Name'Waher( *\w+)*)\s*(?'From'\d{4})(-(?'To'\d{4}))?" -x "C:\Temp\1.xml" -s -o -n -r "Copyright © ${Name} ${From}-2026"
		/// 
		/// Updating copyright statements in markdown files:
		/// Waher.Utility.RegEx.exe -p "C:\My Projects\IoTGateway\*.md" -f "\([cC]\) \[(?'Name'Waher( *\w+)*)\]\((?'Url'[^)\n]*)\)\s*(?'From'\d{4})(-(?'To'\d{4}))?" -x "C:\Temp\2.xml" -s -o -n -r "(c) [${Name}](${Url}) ${From}-2026"
		/// 
		/// Updating copyright statements in RTF files:
		/// Waher.Utility.RegEx.exe -p "C:\My Projects\IoTGateway\*.rtf" -f "\\'a9 (?'Name'Waher( *\w+)*)\s*(?'From'\d{4})(-(?'To'\d{4}))?" -x "C:\Temp\3.xml" -s -o -n -r "\'a9 ${Name} ${From}-2026"
		/// </example>
