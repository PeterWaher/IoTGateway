...
```javascript
function CreateHTMLTest();
function CreateInnerHTMLTest(ElementId);
```
...
JavaScript files generated from multiple Markdown files on the same page. 
...
which in turn affects the generated JavaScript.
...
do it. This also means, that the script syntax normally used for single-braces evaluation on the server, is not used in the JavaScript case.
...
var A=5;
var B=7;
CreateInnerHTMLTestTable("TestTableLocation");
</script>
```
