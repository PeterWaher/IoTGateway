...
| `PushFirst(x,v)`                                       | Adds a new element to a vector `v`, increasing its dimension, adding the element as the first element. If `v` is a variable reference, the variable will contain the new vector. | `PushFirst(x,v)` |
| `PushLast(x,v)`                                        | Adds a new element to a vector `v`, increasing its dimension, adding the element as the last element. If `v` is a variable reference, the variable will contain the new vector. | `PushLast(x,v)` |
...
| `FlushCounters()`                                        | Flushes any pending counters down to the database. Returns the number of counters that were flushed to the database. | `FlushCounters()` |
...
| `GetCounters([Prefix[,Delimiter[,Wildcard]]])`           | Gets a dictionary containing matching counters and their current counts. To ensure all current counters pending to be persisted are included, call `FlushCounters()` first. | `GetCounters("Test.*",".","*")` |
...
| `GetHashObject([Realm,]Hash)`                            | Verifies a Hash digest in an optional realm. The function returns the associated object value if the hash was found and `null` if not. | `GetHashObject("Secrets",Sha2_256(Secret))` |
...
| `PersistHash([Realm,[Expires,[Object,]]]Hash)`           | Persists a Hash digest in an optional realm. The function returns `true` if the hash was persisted and `false` if it was already persisted. The hash can be provided an expiry timestamp, after which the hash will be removed. The has value can also have an optional associated object. | `PersistHash("Secrets",Sha2_256(Secret))` |
...
|----------|-------------|
| `ShellExecute(FileName,Arguments,WorkFolder[,TimeoutMs[,LogStandardOutput[,KillOnTimeout]]])` | Starts a process (`FileName` + `Arguments`) in `WorkFolder` and waits for it to exit. `TimeoutMs` is optional; if omitted, a default of 300000 ms (5 minutes) is used; if `0`, no timeout is applied. If `LogStandardOutput` is `false` (default), the function returns the process standard output as a string, and throws a script runtime exception (message from standard error) if the exit code is non-zero. If `LogStandardOutput` is `true`, standard output and standard error are streamed to the event log and the function returns a boolean indicating success (`true` if exit code is 0). If a timeout occurs, a `TimeoutException` is raised and the process is killed if `KillOnTimeout` is `true` (default). If evaluation is cancelled, an `OperationCanceledException` is raised and the process is killed. |
...
| `CountryCode(CountryName)`                            | Looks up a Country Name and returns the corresponding Country Code.                                                                                                            | `CountryCode("Chile")`                                             |
| `CountryCodes()`                                      | Returns a vector of recognized country codes.                                                                                                                                  | `CountryCodes()`                                                   |
| `CountryEntries()`                                    | Returns an object whose property names are country codes, and their corresponding values the corresponding country name.                                                        | `CountryEntries()`                                                 |
| `CountryName(CountryCode)`                            | Looks up a Country Code and returns the corresponding Country Name.                                                                                                            | `CountryName("CL")`                                                |
| `CountryNames()`                                      | Returns a vector of recognized country names.                                                                                                                                  | `CountryCodes()`                                                   |
