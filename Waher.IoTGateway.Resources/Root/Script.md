...
| `Day(TP)`                                                  | Returns the day of a Date and Time value.                                                                |                                                                                                 | `Day(Now)`                                 |
| `Hours(TP)`                                                | Returns the hour of a Date and Time value, or the total number of hours, if the value is a TimeSpan.     |                                                                                                 | `Hours(Now)`                               |
| `Minutes(TP)`                                              | Returns the minute of a Date and Time value, or the total number of minutes, if the value is a TimeSpan. |                                                                                                 | `Minutes(Now)`                             |
| `Month(TP)`                                                | Returns the month of a Date and Time value.                                                              |                                                                                                 | `Month(Now)`                               |
| `Seconds(TP)`                                              | Returns the second of a Date and Time value, or the total number of seconds, if the value is a TimeSpan. |                                                                                                 | `Seconds(Now)`                             |
| `TimeSpan(s)`                                              | Parses a TimeSpan value from a string.                                                                   | Asserts `s` is a time span value.                                                               | `TimeSpan("14:34:12")`                     |
...
| `Year(TP)`                                                 | Returns the year of a Date and Time value.                                                               |                                                                                                 | `Year(Now)`                                |
...
| `Destroy(x)`             | Destroys the value `x`. If `x` is a variable, the variable is also removed. If `x` is a property reference (i.e. `Obj.Property`), and the property can be removed, it is also removed. The result shows if the property was removed.       |                                                        | `Destroy(x)` |
...
| `MandelbrotSmoothDiffFractal(z,f,dr[,Palette[,DimX[,DimY]]])` | As `MandelbrotFractal`, except the image is smoothed out using the *Heat Equation*, and then the absolute of the differential is applied. Pixels where colors change are used as fixed boundary conditions. | `MandelbrotSmoothDiffFractal((-0.728474426269531,-0.240391845703126),,0.000732421875,RandomLinearRGB(4096,128,1325528060),640,480)`<br/>`MandelbrotSmoothDiffFractal((1.13804443359375,-0.586863875325517),(z,c)->c*(z-z^2),6.103515625E-05,RandomLinearAnalogousHsl(1024,16,21),400,400)` |
| `MandelbrotSmoothDiff2Fractal(z,f,dr[,Palette[,DimX[,DimY]]])` | As `MandelbrotFractal`, except the image is smoothed out using the *Heat Equation*, and then the absolute of the differential is applied twice. Pixels where colors change are used as fixed boundary conditions. | `MandelbrotSmoothDiff2Fractal((-0.728474426269531,-0.240391845703126),,0.000732421875,RandomLinearRGB(4096,128,1325528060),640,480)`<br/>`MandelbrotSmoothDiff2Fractal((1.13804443359375,-0.586863875325517),(z,c)->c*(z-z^2),6.103515625E-05,RandomLinearAnalogousHsl(1024,16,21),400,400)` |
...
| Function                                             | Description | Example |
|------------------------------------------------------|-------------|---------|
| `AuthenticateJwt(Request,Realm,Users[,EncStrength])` | Performs Bearer and JWT authentication of the user of the request. If user is not authenticated, and appropriate exception is thrown and error returned to the client. If authentication is performed, authenticated user object is returned. `EncStrength` can be a boolean (for if encryption is required yes/no) or a positive integer value, mening the minimum cipher strength requried for the mechanism. | `AuthenticateJwt(Request,Gateway.Domain,Waher.Security.Users.Users.Source,128)` |
...
| `ReadSensorData(Sensor[,Types[,Fields[,From[,To]]]])` | Reads a sensor defined on the gateway. The response is an object of the type `{"Error":boolean,"Ok":boolean,"Fields":Dictionary<string,Field>,"Errors":ThingError} |
